...
def control_drone(ip, port=8889):
    ...
    for command in commands:
        ...
        #response = send_with_retry(command, addr)
    ...
    while True:
        if keyboard.is_pressed("left"):
            print("Клавиша < нажата. Дрон летит Вперед...")
            response = send_command("forward 100", addr)
            if response is not None:
                print(f"Ответ от дрона: {response}")
        if keyboard.is_pressed("right"):
            print("Клавиша > нажата. Дрон летит НАЗАД...")
            response = send_command("back 100", addr)
            if response is not None:
                print(f"Ответ от дрона: {response}")
        if keyboard.is_pressed("up"):
            print("Клавиша > нажата. Дрон летит НАЗАД...")
            response = send_command("flip f", addr)
            if response is not None:
                print(f"Ответ от дрона: {response}")
        if keyboard.is_pressed("down"):
            print("Клавиша ↓ нажата. Посадка дрона...")
            response = send_command("land", addr)
            if response is not None:
                print(f"Ответ от дрона: {response}")
                break  # Выход из цикла управления
...
def main():
    ...
    for ip in drone_ips:
        ...
        control_drone(ip) # start drone. Takeof
